...
class FeedForwardECMoe(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        g, m = torch.topk(
            probs.permute(0, 2, 1), tokens_per_expert, dim=-1
        )  # [n, e, k], [n, e, k]
...
class DiTBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = (
            FeedForwardECMoe(
                num_experts, expert_capacity, dim, mlp_hidden_dim, multiple_of
            )
            if moe_block
            else FeedForward(dim, mlp_hidden_dim, multiple_of, use_bias)
        )
    ...
    def forward(
        self, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor, **kwargs
    ) -> torch.Tensor:
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        if self.use_patch_mixer:
            expert_blocks_idx = [
                i for i in range(1, patch_mixer_depth) if (i + 1) % experts_every_n == 0
            ]
            is_moe_block = [
        ...
        if len(ffn_multipliers) == depth:
            ...
            assert (
                depth % num_splits == 0
            ), "number of blocks should be divisible by number of splits"
            depth_per_split = depth // num_splits
            qkv_ratios = list(
                np.array([[m] * depth_per_split for m in qkv_multipliers]).reshape(-1)
            )
            mlp_ratios = list(
                np.array([[m] * depth_per_split for m in ffn_multipliers]).reshape(-1)
            )

        # Don't use MoE in last block
        expert_blocks_idx = [
            i for i in range(0, depth - 1) if (i + 1) % experts_every_n == 0
        ]
        ...
        self.register_buffer(
            "mask_token", torch.zeros(1, 1, patch_size**2 * self.out_channels)
        )
    ...
    def forward_without_cfg(
        ...
        **kwargs,
        ...
        y = self.y_emb_preprocess(y.squeeze(dim=1)).unsqueeze(
            dim=1
        )  # (N, 1, L, D) -> (N, D)
